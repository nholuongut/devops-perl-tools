...
if echo "select columns[0] from myTable where name = 'nholuongut/emoji-cheat-sheet';" | $perl -T ./drillcase.pl | tee /dev/stderr | grep -qF "SELECT columns[0] FROM myTable WHERE name = 'nholuongut/emoji-cheat-sheet';"; then
